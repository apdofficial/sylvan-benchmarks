...
def load_quality_df(path: Path) -> DataFrame:
    ...
    for path in quality_results:
        ...
        n, nt, mg, mv, ms, rt, model = path.name.split("-")
        frame["workers"] = n
...
def generate_quality_table():
    ...
    quality.to_csv(cwd / "sylvan_regression_quality.csv", encoding='utf-8', index=False)
...
def generate_runtime_table():
    ...
    runtime.to_csv(cwd / "sylvan_regression_runtime.csv", encoding='utf-8', index=False)
