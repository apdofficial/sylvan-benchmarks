...
for model in "${add_auto_models[@]}"
do
   ./hyperfine \
     -L reordering-trigger "sa" \
     -L solver "sylvan-solver -n 8 --nt 1 --tr 2 --ts 21 --mg 1.15 --mv 200 --ms 10000","cudd-solver --h="sift"","cudd-solver --h="group"" \
     -L model $MODELS_PATH/"$model".aag \
     --runs 10 \
     --warmup 1 \
     './{solver} --rt={reordering-trigger} {model}' \
...
for model in "${mult_auto_models[@]}"
do
   ./hyperfine \
     -L reordering-trigger "sa" \
     -L solver "sylvan-solver -n 8 --nt 1 --tr 2 --ts 24 --mg 1.15 --mv 200 --ms 10000","cudd-solver --h="sift"","cudd-solver --h="group"" \
     -L model $MODELS_PATH/"$model".aag \
     --runs 10 \
     --warmup 1 \
     './{solver} --rt={reordering-trigger} {model}' \
